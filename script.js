...
function deleteChat(chatId, event) {
    // Prevent the click from triggering the chat load
    if (event) {
...
function updateSavedChatsList() {
    ...
    sortedChats.forEach(chat => {
        ...
        chatItem.className = `saved-chat-item${chat.id === currentChatId ? ' active' : ''}`;
...
function loadChat(chatId) {
    ...
    // Display all messages with typing effect
    loadChatMessages(conversationHistory);
